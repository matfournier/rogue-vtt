<script>
	...
	onMount( async () => {
		...
		let pattern = context.createPattern(defaultCanvas(defaultTile), "repeat");
	...
	const handleStart = ({ offsetX: x, offsetY: y }) => {
		...
		console.log(`click x: ${selectedMapTile[0]}, y: ${selectedMapTile[1]}, tile: ${tIdx}, tile: ${tileIndex}`);
	...
	const handleMove = ({ offsetX: x1, offsetY: y1 }) => {
		...
		if (selectedMapTile[0] != maybeNewTile[0] || selectedMapTile[1] != maybeNewTile[1]) {
